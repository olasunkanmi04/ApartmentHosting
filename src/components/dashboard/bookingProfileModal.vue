      ...
      <div class="modal__body">
        <div class="modal__profile-wrap">
          ...
          <div class="modal__profile-wrapper">
            ...
            <div class="modal__profile-detail">
              <p class="modal__profile-name">{{booking.name}}</p>
              <p class="modal__profile-role">{{booking.role}}</p>
              <p class="modal__profile-location">{{booking.location}}</p>
            </div>
          </div>
        </div>
        <div class="modal__property">
          <div class="modal__property-info">
            ...
            <span class="modal__property-name">{{booking.spaceName}}</span>
          </div>
          <div class="modal__property-info">
            <span class="modal__property-title">Location</span>
            <span class="modal__property-name">{{booking.location}}</span>
          </div>
          <div class="modal__property-info">
            <span class="modal__property-title">Property Type</span>
            <span class="modal__property-name">{{booking.propertyType}}</span>
          </div>
          <div
            class="modal__property-info"
            v-if="booking.propertyType && booking.propertyType == 'shared'"
          >
            <span class="modal__property-title">Bedroom Name</span>
            <span class="modal__property-name">{{booking.bedroomName}}</span>
          </div>
          <div class="modal__property-info">
            <span class="modal__property-title">Duration</span>
            <span class="modal__property-name">{{booking.duration}}</span>
          </div>
          <div class="modal__property-info">
            <span class="modal__property-title">Cost of Subscription</span>
            <span class="modal__property-name font-weight-bold">{{booking.cost}}</span>
          </div>
          <div class="modal__action float-right">
            <button
              type="button"
              class="cta__btn mr-4"
              @click="deleteModalShow(booking.name)"
            >Accept</button>
            <button type="button" class="declineBtn" @click="declinetModalShow">Decline</button>
          </div>
        </div>
        <div class="modal__decline-opt">
          <declineModal :name="name"></declineModal>
        </div>
      </div>
    </div>
  </modal>
